...
const App = () => {
  ...
  const handleForm = (event) => {
    ...
    if (persons.some(person => person.name === newName)){
      ...
      const contact = {...oldContact, number: newNumber};
      service
        .updateData(contact)
        .then (() => {
          setPersons(persons.map(person => person._id === contact._id ? contact:person));
          setPersonsToShow(personsToShow.map(person => person._id === contact._id ? contact:person));
          setNewName('');
          setNewNumber('');
  ...
  const handleDelete = (id, name) => {
    ...
    service
      ...
      .then(() => {
        ...
        setPersonsToShow(personsToShow.filter(persons=> persons._id!=id));
        setNotificationType('success');
